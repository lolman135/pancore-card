...
export function coilSketch() {
  ...
  <text class="spec" x="114" y="14" text-anchor="middle">Ø121</text>
  <line class="dim" x1="${ix0}" y1="${y1 + 10}" x2="${ix1}" y2="${y1 + 10}" marker-end="url(#sk-ar)"/>
  <text class="spec" x="114" y="${y1 + 22}" text-anchor="middle">Ø53</text>
  <line class="dim" x1="${x1 + 12}" y1="${y0}" x2="${x1 + 12}" y2="${y1}" marker-end="url(#sk-ar)"/>
  <text class="spec" x="${x1 + 24}" y="106" transform="rotate(-90 ${x1 + 24} 106)" text-anchor="middle">H 213.8</text>
  <line class="dim" x1="${x0 - 10}" y1="${y0}" x2="${x0 - 10}" y2="${ix0 - x0 + y0}"/>
  <text class="spec" x="${x0 - 14}" y="${y0 + 20}" transform="rotate(-90 ${x0 - 14} ${y0 + 20})" text-anchor="middle">34</text>
  <text class="spec" x="8" y="${y1 + 22}">SFC-30 · 2245 ${tx('г', 'g')}</text>
...
export function skySketch() {
  ...
  <text class="spec" x="140" y="56" text-anchor="end">MH1.25 5P</text>
  <line class="dim" x1="58" y1="38" x2="170" y2="38" marker-end="url(#sk-ar)"/><text class="spec" x="114" y="34" text-anchor="middle">42 × 42 · 15 ${tx('мм', 'mm')}</text>
  <text class="spec" x="92" y="184">FC · 1310/1550 ${tx('нм', 'nm')} · ≤16 ${tx('г', 'g')}</text>`);
}
...
export function groundSketch() {
  ...
  <path class="ln2" d="M196,106 L214,106"/><rect class="ln" x="214" y="98" width="10" height="16"/><text x="200" y="124">XT60</text>
  <line class="dim" x1="34" y1="44" x2="196" y2="44" marker-end="url(#sk-ar)"/><text class="spec" x="115" y="40" text-anchor="middle">95 × 62 · 48 ${tx('мм', 'mm')}</text>
  <text class="spec" x="34" y="180">2 × ${tx('відео · тренер DSC', 'video · DSC trainer')} · 112 ${tx('г', 'g')}</text>`);
}
...
export function hookupSketch() {
  ...
  <text x="176" y="93">${tx('оптична розетка', 'optical socket')}</text><text class="spec" x="176" y="102">${tx('1310/1550 нм · ≤ 80 км', '1310/1550 nm · ≤ 80 km')}</text>
  <rect class="ln" x="222" y="46" width="14" height="6"/><path class="ln" d="M229,46 v-14"/>
  <rect class="ln2 fillDim" x="206" y="12" width="46" height="20" rx="3"/><text x="212" y="25">XT60 · ${tx('АКБ', 'battery')}</text>
...
/* data-plain: схема із загальними позначеннями, але без характеристик приладу —
   підписи з class="spec" (розміри, маса, довжини хвиль), телеметрія (osd) і розмірні лінії прибираються. */
const plain = (svg) => svg
  .replace(/<text\b[^>]*class="(?:spec|osd)[^"]*"[^>]*>[\s\S]*?<\/text>/g, '')
  .replace(/<(?:line|path|rect|circle|polygon)\b[^>]*class="dim[^"]*"[^>]*\/>/g, '');
...
export function mountSketches(root = document) {
  root.querySelectorAll('[data-sketch]').forEach((el) => {
    ...
    const svg = fn(...args);
    el.innerHTML = el.hasAttribute('data-plain') ? plain(svg) : svg;
  });
}
